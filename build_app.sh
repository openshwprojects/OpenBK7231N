...
cd $APP_PATH
if [ -f build.sh ]; then
    sh ./build.sh $APP_NAME $APP_VERSION $TARGET_PLATFORM $USER_CMD $BUILD_MODE
elif [ -f Makefile -o -f makefile ]; then
    export COMPILE_PREX TARGET_PLATFORM
    make APP_BIN_NAME=$APP_NAME APP_NAME=$APP_NAME APP_VERSION=$APP_VERSION USER_SW_VER=$APP_VERSION USER_CMD=$USER_CMD BUILD_MODE=$BUILD_MODE all
    ...
    sh $TUYA_APPS_BUILD_CMD $APP_NAME $APP_VERSION $TARGET_PLATFORM $USER_CMD $BUILD_MODE
else
